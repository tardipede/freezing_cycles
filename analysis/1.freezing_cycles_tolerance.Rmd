...
```{r echo=T}
means_cycles = aggregate(chains.da1$M, by=list(chains.da1$species_code), FUN=mean)
colnames(means_cycles) = c("species_code", "M")
...
chains_toplot = chains.da1[,c(6,3)]

# load the tree
tree = chronos(read.nexus("./data/tree.nex"))
tree = drop.tip(tree, tip = tree$tip.label[!(tree$tip.label %in% unique(chains_toplot$species))])
class(tree) = "phylo"
...
T50plot = ggplot(chains_toplot)+
  ...
  scale_y_continuous(trans="log2",breaks = c(1,2,6,11,21,41,81), labels =c(0,1,5,10,20,40,80))+
...
ggsave("./output/tree_Cycles.svg")

```

Download the plot in svg format
```{r echo=FALSE}
embed_file('./output/tree_Cycles.svg')
```
...
It seems there is no phylogenetic signal in the resistance to freeze-thaw cycles, however the sample is low (11 species), so those results should be taken cautiously.
