...
## Get bioclim data for species sampling locations
```{r get bioclim}
...
## Run PCA on Bioclidata and calculate simplfied Principal Components
See @Bjorklund2019 for more information on simplified Principal Components (sPCs)
```{r simplify PCA}
...
# Given the p-values, we will retain only the first 2 PCs
...
cor.PC1 = cor.test(sPC1,pca_bioclims$x[,1]) 

# test if PC2 and sPC2 are correlated
cor.PC2 =cor.test(sPC2,pca_bioclims$x[,2])
...
```
The correlation between PC1 and sPC1 is `r round(cor.PC1$estimate,3)`, whereas the correlation between PC2 and sPC2 is `r round(cor.PC2$estimate,3)`, so we can safely use the simplified PCs instead of the original ones.  

## Merge sPCA data with freezing cycles tolerance data
...
## Plot beta estimates and sPCAs loadings for final figure
```{r plot, results='hide',fig.keep='all'}
...
plot1 = ggplot(chains_to_plot )+
  ...
  scale_x_discrete(labels = c("sPC2","sPC1"))+ 
  ggtitle("Effect of sPCs on \nfreezing-thawing resistance")
...
plot2 = ggplot(sloadings)+
  ...
  theme(panel.grid.major.y = element_blank(), panel.grid.minor = element_blank(),panel.border =    element_blank(), axis.text.x = element_text(angle = 90))+
  scale_x_discrete(label = paste0("BIO",1:11))
...
plot1|(plot2/plot3)
ggsave("./output/plot_mod_fitenv.pdf")
```
Download plot in pdf here:
```{r echo=FALSE}
embed_file('./output/plot_mod_fitenv.pdf')
```

The sPC1 has not a significant effect on the number of freeze-thawing cycles to death of 50% of individuals (p = `r round(p_vals[2],4)`), whereas sPC2 has an effect (p = `r round(p_vals[3],4)`).  
The contributors of sPC2 are **BIO6**, **BIO9** and **BIO10**, and they all contribute positively to it, whereas the coefficent for the effect of sPC2 is negative, so those three variables have a negative effect on the freezing cycles tolerance.  It should be noted that it is not possible to distinguish between the effect of the single BIOclim variables, as in our data they are correlated.  As general result, the tolerance to freezing cycles is negatively impacted by the minimum temperature of the coldest month, by the mean temperatures of the driest and of the warmest quarters.  
